...
# setup_legacy_vm.sh
#
# Levanta 1 VM KVM/libvirt (Ubuntu 18.04 cloud image) para el lab de Ansible legacy.
# Genera un inventario estático YAML en el directorio de trabajo actual.
#
# Uso:
#   chmod +x setup_legacy_vm.sh
#   ./setup_legacy_vm.sh
#
# Requisitos:
...
# Nombres de las VMs
VM_NAMES=("server-legacy")

# Recursos por VM (WordPress LAMP en single node requiere más recursos)
VM_CPUS=2
VM_RAM_MB=2048
VM_DISK_GB=10
...
BASE_IMAGE_NAME="bionic-server-cloudimg-amd64.img"   # Ubuntu 18.04
...
INVENTORY="${PWD}/hosts_legacy.yml"
...
check_prerequisites() {
    ...
    if [[ -z "${BASE_IMAGE}" ]]; then
        ...
        echo ""
        echo "    Descárgala con:"
        echo "      wget -O ${BASE_IMAGE_DIR}/${BASE_IMAGE_NAME} https://cloud-images.ubuntu.com/bionic/current/${BASE_IMAGE_NAME}"
...
### Para AAP el inventario no debe tener usuario ni credenciales — se configuran desde AAP
#   ansible_user: ${SSH_USER}
#   ansible_ssh_private_key_file: ${SSH_KEY}

  hosts:
    server-legacy:
      ansible_host: ${VM_IPS[server-legacy]}
EOF
...
print_summary() {
    ...
    echo -e "${COLOR_BOLD}${COLOR_GREEN}  Lab legacy listo${COLOR_RESET}"
    echo -e "${COLOR_BOLD}${COLOR_GREEN}======================================================${COLOR_RESET}"
    printf "  ${COLOR_BOLD}%-15s %s${COLOR_RESET}\n" "VM" "IP"
    printf "  %-15s %s\n" "---------------" "---------------"
    for vm in "${VM_NAMES[@]}"; do
        printf "  ${COLOR_GREEN}%-15s %s${COLOR_RESET}\n" "${vm}" "${VM_IPS[${vm}]}"
    done
    echo ""
    echo -e "  Inventario : ${COLOR_CYAN}${INVENTORY}${COLOR_RESET}"
    echo -e "  Usuario SSH: ${COLOR_CYAN}${SSH_USER}${COLOR_RESET}"
    echo -e "  OS         : ${COLOR_CYAN}Ubuntu 18.04 LTS (Bionic)${COLOR_RESET}"
    echo -e "${COLOR_BOLD}${COLOR_GREEN}======================================================${COLOR_RESET}"
    echo ""
}
...
echo ""
msg_info "Eliminando VM existente si la hay..."
for vm in "${VM_NAMES[@]}"; do
    delete_vm "${vm}"
done

echo ""
msg_info "Creando VM legacy..."
for vm in "${VM_NAMES[@]}"; do
    echo ""
    msg_section "${vm}"
